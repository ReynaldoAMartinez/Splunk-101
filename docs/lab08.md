...
I’ll create an alert to be a brute force activity
...
I introduced the command  bucket that is similar to time chart, where I am instructing Splunk to slice the data depending on the time span that I input here.
...
Added a counter greater than 100
...
For the Cron Expression, I used a Cron schedule from the link https://crontab.cronhub.io/
...
I see the query and data that is responsible for this alert
...
Now there are not more alerts because it was disabled. Once it is disabled, Splunk removes all the previous alerts from the list.
